...
Prototype app that lets a server (via ntfy.sh notifications) drive UI on a
Pebble smartwatch. An Android companion app intercepts ntfy broadcasts and
forwards them to the watch over Bluetooth.

See the blog post [here](https://upvalue.io/posts/pebble-smartwatch-server-driven-ui/).

Inspired by bitbar, xbar etc. Put arbitrary things on the watch without writing
task-specific C/Java.
